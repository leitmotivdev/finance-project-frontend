...
export const CardWallet: FC<CardWalletProps> = ({ wallet }) => {
  return (
    <button className="card-wallet">
      <div style={{ background: wallet.accountColor }} className="card-wallet__color-bar"></div>
      <div className="card-wallet__content">
        <p className="card-wallet__name">{wallet.accountName}</p>
        <p className="card-wallet__number">{wallet.accountNumber}</p>
      </div>

      <p className="card-wallet__balance">{parseCurrency(wallet.accountBalance)}</p>
    </button>
  );
};